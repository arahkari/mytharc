...
public class ComponentDarkDamage extends SpellEffect implements IDamageComponent {

    public ComponentDarkDamage(ResourceLocation icon) {
        super(icon, new AttributeValuePair[]{new AttributeValuePair(Attribute.DAMAGE, 2.0F, 2.0F, 20.0F, 1F, 3.0F), new AttributeValuePair(Attribute.LESSER_MAGNITUDE, 1.0F, 0.0F, 6.0F, 1.0F, 3.0F)});
    }
    ...
    public ComponentApplicationResult ApplyEffect(SpellSource source, SpellTarget target, IModifiedSpellPart<SpellEffect> modificationData, SpellContext context) {
        ...
        if(target.isEntity() && target.getEntity() instanceof LivingEntity entity)
        {
            ...
            damage *= (20 / spellCaster.getHealth());

            if(spellCaster.getHealth() <= 2.0 + modificationData.getValue(Attribute.LESSER_MAGNITUDE))
            {
                damage += modificationData.getValue(Attribute.LESSER_MAGNITUDE);
            }
